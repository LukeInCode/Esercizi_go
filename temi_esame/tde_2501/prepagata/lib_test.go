...
func ConfrontaConOracolo(
	...
	if _, err := os.Stat(progname); err != nil {
		fmt.Println("ERRORE >>> Manca eseguibile studente!")
		t.Fail()
		return
	}
	...
	if _, err := os.Stat(fileoracolo); err != nil {
		fmt.Println("ERRORE >>> Manca oracolo!")
		t.Fail()
		return
	}
	...
	if e != nil {
		fmt.Println("ERRORE >>> FAIL! differisce da output atteso")
		t.Fail()
	}
...
func Diff2files(fn1, fn2 string) (e error) {
	cmd := exec.Command("") // non trova *Cmd (indagare)
	//cmd := exec.Command("diff", "-y", fn1, fn2)
	//cmd := exec.Command("diff", "-y", "--ignore-trailing-space", "-W", fmt.Sprint(diffwidth),"--color=always","--suppress-common-lines", fn1, fn2) // su MAC non c'è opzione in versione estesa
	/* prova con variazione se lancio io
	u, _ := user.Current()
	if strings.Contains(u.Username, "trent") {
		fmt.Println("< atrent diffing >")
		cmd = exec.Command("diff", "-y", "-W", fmt.Sprint(diffwidth), fn1, fn2)
	} else {
		cmd = exec.Command("diff", "-y", "-W", fmt.Sprint(diffwidth), "-b", fn1, fn2) // così ignora OGNI whitespace diff
	}*/

	// -b = ignore changes in the amount of white space

	/*if diffwidth > 100 {
		cmd = exec.Command("diff", "--color=always", "-W", fmt.Sprint(diffwidth), fn1, fn2)
	} else {*/

	cmd = exec.Command("diff", "-y", "-W", fmt.Sprint(diffwidth), fn1, fn2)
	//NON VA SEMPRE BENE cmd = exec.Command("diff", "--color=always", "-y", "-W", fmt.Sprint(diffwidth), fn1, fn2)
	//cmd = exec.Command("icdiff", "--cols=", fmt.Sprint(diffwidth), fn1, fn2) // prova con icdiff
