...
**NOTA BENE**: le presenti istruzioni e tutti gli script forniti si basano su sistema operativo GNU/Linux e presumono una certa abilità dello studente nell'uso della shell (bash in particolare).
...
La maggior parte degli editor fornisce la funzionalità di "wordwrapping" (a capo automatico) dei file.



## Espandere l'archivio con le specifiche

Lanciando il comando `tar xvf <nome del file>.tar` viene espanso l'archivio a partire dalla directory in cui ci si trova (a patto che vi sia stato scaricato il file stesso, cioè che sia presente il file .tar nella directory corrente).

Verrà creata una sottodirectory per ogni esercizio con tutto il materiale occorrente per svolgerlo (specifiche, test, ecc.)

Si invitano gli studenti a lavorare nelle sottodirectory così create.



## Contenuto della directory del tema d'esame
...
Nota bene: per effettuare il build serve il file `go.mod` che viene creato con `go mod init <nome modulo>` (il <nome modulo> è il nome del programma che dovete realizzare).

Ricordarsi di ricompilare dopo ogni modifica prima di lanciare di nuovo i test.

Alcuni test producono un `diff` tra l'esercizio dello studente e l'"oracolo" (l'output atteso), il formato è su due colonne, a sinistra l'output studente e a destra l'oracolo; i simboli in mezzo alle due colonne evidenziano eventuali discrepanze:
 "<" la riga appare solo a sinistra
 ">" la riga appare solo a destra
 "|" le righe destra e sinistra differiscono (tipicamente per whitespace)
 "\" o "/" le righe destra e sinistra differiscono (tipicamente per un newline in fondo)
...
Va effettuata caricando su http://upload.di.unimi.it, utilizzando la propria login e scegliendo la SESSIONE CORRETTA (nel dubbio chiedere ai docenti, consegnare in una sessione sbagliata comporta la perdita della consegna e il conseguente annullamento dell'esame):
- i SINGOLI file `<nome esercizio>.go` (quindi NON vanno consegnati gli eseguibili, i test e gli altri file)
- per ogni esercizio almeno un `output` dell'esecuzione dei test dando come nome di file `<nome esercizio>.student-test` (suggerimento: per redirigere l'output di un comando potete leggere la sezione REDIRECTION del manuale della `bash`)
...
**NOTA BENE**: la sessione (login) di upload scade dopo circa 15 minuti di inattività, quindi nel caso occorre semplicemente fornire di nuovo le proprie credenziali (evitare di trovarsi in questa situazione a un minuto dall'ora di consegna).
...
E` pregato semplicemente di scollegarsi dalla sessione di upload, senza spegnere il PC, e di passare dal docente prima di lasciare l'aula.



## >>> Valutazione <<<

**ATTENZIONE**, condizioni necessarie perché un esercizio (un sorgente .go) consegnato venga valutato dai docenti:
- il sorgente deve compilare
- il sorgente dei TEST forniti deve compilare (i.e., `go test` non deve dare un errore di compilazione)
- il file con l'output del test effettuato dallo studente deve essere consegnato (col nome corretto) e deve contenere l'effettivo risultato di un lancio dei test (anche con dei FAIL, e non occorre che sia quello relativo all'ultima versione del programma)

Un sorgente che non rispetti TUTTE le condizioni sopra elencate NON verrà nemmeno esaminato, è come se non fosse stato consegnato.

Inoltre, affinché tutto l'elaborato venga valutato, è richiesto che l'esercizio FILTRO (o altrimenti un altro esercizio tra quelli assegnati) compili e produca l'output atteso, come da specifiche. A questo proposito i docenti si riservano di eseguire ulteriori test rispetto a quelli messi a disposizione durante l'esame.


Alla formazione del voto concorreranno poi i seguenti aspetti:
...
- l'uso corretto delle strutture di dati e di controllo
